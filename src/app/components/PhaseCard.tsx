...
export function PhaseCard({ phase, onClick, onNameChange, listeners }: PhaseCardProps & { listeners?: any }) {
  ...
  return (
    <Card
      className="p-4 hover:shadow-md transition-all cursor-pointer border border-gray-200 w-full flex-shrink-0 relative overflow-hidden group select-none bg-white"
      onClick={(e) => {
        if (!isEditing) {
          onClick();
        }
